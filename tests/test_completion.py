...
def _repository(
    ...
    *,
    ...
    lifecycle = {name: dict(completed_stage) for name in completed_stage_names}
    lifecycle["reuse"] = {
...
def _read_all(paths: tuple[Path, ...]) -> list[bytes]:
    return [path.read_bytes() for path in paths]


def test_complete_execution_closes_goal_and_readies_next_goal(
    tmp_path: Path,
    ...
    assert lifecycle["reuse"]["status"] == "completed"
    assert completion["disposition"] == "goal-completed"


def test_complete_execution_rejects_non_reuse_stage_without_changes(
    tmp_path: Path,
) -> None:
    ...
    assert _read_all(paths) == originals
...
def test_complete_execution_validation_rejection_is_atomic(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repository, state_path, goal_path, next_goal_path, execution_path = _repository(tmp_path)
